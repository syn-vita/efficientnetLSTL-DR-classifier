...
def main(image_dir: str, csv_path: str, out_dir: Optional[str] = None):
    ...
    for fold_num, (train_index, val_index) in enumerate(skf.split(X, y), 1):
        ...
        # Persist checkpoints for downstream testing in both Torch (.pth) and ONNX formats
        pth_save_path = os.path.join(PROJECT_FOLDER, f"efficientnet_b0_lstl_clean_fold_{fold_num}.pth")
        try:
